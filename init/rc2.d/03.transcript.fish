...
function crm_opp_activity_by_id -a id -d 'pretty list of activity on an opportunity'
    ...
    fdt_ls -t opportunity | jq -r '. | select(.id == "'$id'") | if .event_type == "opportunity-new" then '$fields_new' elif  .event_type == "opportunity-update" then '$fields_update' else '$fields_default' end | join("\t\t")' | sort -r | sed 's/PP/\n/g'
end
